...
if (file_exists($file)) {
    ...
    while( ($line = fgetcsv($filepath)) !==FALSE) {
        ...
        $insert = "INSERT INTO FHNI_Dogs (name, age, sex, breed, adoptableFrom, posted, color, coat, size, neutered, housebroken, likesPeople, likesChildren, 
                        getAlongMales, getAlongFemales, getAlongCats, keepin, postcode) 
                            VALUES ( '{$line[0]}','{$line[1]}','{$FK}','{$line[3]}','{$line[4]}','{$line[5]}','{$line[6]}','{$line[7]}','{$line[8]}'
